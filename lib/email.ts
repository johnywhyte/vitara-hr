      ...
      <table width="100%" style="max-width:560px;background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 1px 6px rgba(0,0,0,0.08);">
        ...
        <tr>
          <td style="background:#F8F9FA;padding:14px 28px;border-top:1px solid #E9ECEF;text-align:center;">
            <p style="margin:0;font-size:10px;color:#6C757D;">© 2026 Vitara Agricultural E-Commerce · Ghana · <a href="${BASE_URL}" style="color:#2D6A4F;">vitara.ag</a></p>
          </td>
        </tr>
      </table>
    </td>
...
export async function sendApplicationSubmittedEmail(to: string, name: string) {
  return getResend().emails.send({
    ...
    html: baseHtml(`
      <h2 style="margin:0 0 8px;color:#2D6A4F;font-size:18px;">Application Received!</h2>
      ...
      <div style="background:#D8F3DC;border-left:3px solid #52B788;border-radius:6px;padding:12px 16px;margin:0 0 20px;">
        <p style="margin:0;color:#2D6A4F;font-size:13px;font-weight:600;">What happens next?</p>
        <p style="margin:6px 0 0;color:#2D6A4F;font-size:12px;">Our team will review your application within 3–5 business days. You will receive an email notification once a decision has been made.</p>
      </div>
      <a href="${BASE_URL}/apply/step3" style="display:inline-block;background:#2D6A4F;color:#fff;text-decoration:none;padding:10px 20px;border-radius:6px;font-size:13px;font-weight:600;">View Application</a>
    `),
  })
}
...
export async function sendApplicationApprovedEmail(to: string, name: string) {
  return getResend().emails.send({
    ...
    html: baseHtml(`
      <div style="background:#2D6A4F;border-radius:8px;padding:24px;text-align:center;margin-bottom:20px;">
      ...
      <div style="background:#D8F3DC;border-left:3px solid #52B788;border-radius:6px;padding:12px 16px;margin:0 0 20px;">
        <p style="margin:0;color:#2D6A4F;font-size:13px;font-weight:600;">Next Steps</p>
        <p style="margin:6px 0 0;color:#2D6A4F;font-size:12px;">You will receive a separate email shortly with details regarding your interview schedule or onboarding process. Please keep an eye on your inbox and ensure you check your spam folder too.</p>
      </div>
...
export async function sendApplicationRejectedEmail(
  ...
  return getResend().emails.send({
    ...
    html: baseHtml(`
      ...
      <a href="${BASE_URL}/apply/step1" style="display:inline-block;background:#2D6A4F;color:#fff;text-decoration:none;padding:10px 20px;border-radius:6px;font-size:13px;font-weight:600;">View Application</a>
